...
LALRParser::LALRParser(::mpl::Reader& reader) :
		_lexer(reader) {
	if (!s_init) {
		init();
		s_init = true;
...
void LALRParser::parse() {
	...
	while (!st.empty()) {
		...
		const LALRGrammar::Tran& tran = s_grammar[state];
		...
		LALRGrammar::Tran::const_iterator it = tran.find(token);
		assert(it != tran.end());
		if (it->second.first < 0) {
			int left = it->second.first;
			int rule_no = it->second.second;

			const LALRGrammar::InnerRule& rule = s_grammar.rule(left, rule_no);
